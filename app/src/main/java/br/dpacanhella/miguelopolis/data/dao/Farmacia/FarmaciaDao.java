...
public class FarmaciaDao {
    ...
    public void getAllUtilitarios(String anuncio, Callback callback) throws DaoException {
        wsClient.getAllUtilitarios(anuncio).enqueue(callback);
    }
    ...
    public List<TipoAnuncio> getAllAnuncios() throws DaoException {
        try {
            Call<List<TipoAnuncio>> call = wsClient.getAllAnuncios();
            Response<List<TipoAnuncio>> response = call.execute();
            if (response.isSuccessful()) {
                return response.body();
            } else {
                throw new DaoException(response.code(), response.message());
